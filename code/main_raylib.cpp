// main_raylib.cpp
#include <algorithm>
#include <iostream>   // for cout in catch
...
struct Textures {
  ...
  Texture2D foodEssence{};
  Texture2D foodElixir{};
  Texture2D foodPotion{};
...
static void LoadAllTextures() {
  ...
  gTex.foodEssence = LoadTexture("assets/tripleEssence.png");
  gTex.foodElixir = LoadTexture("assets/fireElixir.png");
  gTex.foodPotion = LoadTexture("assets/healingPotion.png");
...
static void UnloadAllTextures() {
  ...
  UnloadTexture(gTex.foodEssence);
  UnloadTexture(gTex.foodElixir);
  UnloadTexture(gTex.foodPotion);
    ...
    for (int c = 0; c < cols; ++c) {
      ...
      if (auto ft = dynamic_cast<FoodTile*>(t)) {
        ...
        if (f) {
          ...
          if (name == "Triple Essence") {
            DrawInCell(gTex.foodEssence, r, c, cell, margin);
          } else if (name == "Fire Elixir") {
            DrawInCell(gTex.foodElixir, r, c, cell, margin);
          } else {  // Healing Potion
            DrawInCell(gTex.foodPotion, r, c, cell, margin);
          }
        }
      }
  ...
  while (!WindowShouldClose() && !shouldExit) {
    ...
    if (state == GameState::MAIN_MENU) {
      ...
      if (!gm->isGameOver() && !gm->isWin()) {
        ...
        if (IsKeyPressed(KEY_SPACE) || IsKeyPressed(KEY_I))
          gm->handleInput('i');
      }
    ...
    // for error handling to check if blind gui works or not
    ...
    if (state == GameState::MAIN_MENU) {
      ...
      if (gm->getBlindTime() > 0.0) {
        ...
        drewBlind = true;  // flag for error handling to true
      }
    } else if (state == GameState::PAUSED) {
      DrawGame(*gm, cellSize, margin);
      menu.DrawPauseButton();
